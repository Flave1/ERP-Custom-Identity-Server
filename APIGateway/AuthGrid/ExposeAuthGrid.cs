...
namespace APIGateway.AuthGrid
{
    public class ExposeAuthGridHandler : IRequestHandler<ExposeAuthGrid, AuthSettupGridResponder>
    {
        ...
        public ExposeAuthGridHandler(DataContext context, IResponseCacheService cacheService)
        {
            _context = context;
        ...
        public async Task<AuthSettupGridResponder> Handle(ExposeAuthGrid request, CancellationToken cancellationToken)
        {
            ...
            response.AuthSettups = lockoutSetting.GroupBy(r => r.Module).Select(t => t.First()).Select(q => new AuthSettup
            {
