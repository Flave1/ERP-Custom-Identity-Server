...
{
    public class DeleteStaffCommandHandler : IRequestHandler<DeleteStaffCommand, DeleteRespObj>
    {
        private readonly IAdminRepository _adminRepo;
        private readonly ILoggerService _logger;
        private readonly DataContext _dataContext;
        public DeleteStaffCommandHandler(IAdminRepository adminRepository, DataContext dataContext, ILoggerService loggerService)
        {
            _adminRepo = adminRepository;
            _dataContext = dataContext;
            _logger = loggerService;
        }
        public async Task<DeleteRespObj> Handle(DeleteStaffCommand request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                if (request.req.Count() > 0)
                {
                    ...
                    {
                        await _adminRepo.DeleteStaffAsync(itemId.TargetId);
                    }
                }
                return response;
            }
