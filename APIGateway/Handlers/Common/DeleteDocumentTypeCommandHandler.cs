...
{ 
    public class DeleteDocumentTypeCommandHandler : IRequestHandler<DeleteDocumentTypeCommand, DeleteRespObj>
    {
        private readonly ICommonRepository _repo;
        public DeleteDocumentTypeCommandHandler(ICommonRepository commonRepository)
        {
            _repo = commonRepository;
        }
        public async Task<DeleteRespObj> Handle(DeleteDocumentTypeCommand request, CancellationToken cancellationToken)
        {
            try
            {
                if (request.ItemsId.Count() > 0)
                {
                    ...
                    {
                        await _repo.DeleteDocumentTypeAsync(itemId);
                    }
                }
