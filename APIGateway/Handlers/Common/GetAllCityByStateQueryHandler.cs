...
namespace APIGateway.Handlers.Common
{
    ...
    public class GetAllCityByStateQueryHandler : IRequestHandler<GetAllCityByStateQuery, CommonLookupRespObj>
    {
        private readonly ICommonRepository _repo;
        public GetAllCityByStateQueryHandler(ICommonRepository commonRepository)
        {
            _repo = commonRepository;
        }
        public async Task<CommonLookupRespObj> Handle(GetAllCityByStateQuery request, CancellationToken cancellationToken)
        {
            var stateList = await _repo.GetAllStateAsync();
            var list = await _repo.GetAllCityByStateAsync(request.StateId);
