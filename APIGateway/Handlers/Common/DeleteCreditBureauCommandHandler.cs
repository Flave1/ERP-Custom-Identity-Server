...
{ 
    ...
    public class DeleteCreditBureauCommandHandler : IRequestHandler<DeleteCreditBureauCommand, DeleteRespObj>
    {
        private readonly ICommonRepository _repo;
        public DeleteCreditBureauCommandHandler(ICommonRepository commonRepository)
        {
            _repo = commonRepository;
        }
        public async Task<DeleteRespObj> Handle(DeleteCreditBureauCommand request, CancellationToken cancellationToken)
        {
            try
            {
                if (request.ItemsId.Count() > 0)
                {
                    ...
                    {
                        await _repo.DeleteCreditBureauAsync(itemId);
                    }
                }
