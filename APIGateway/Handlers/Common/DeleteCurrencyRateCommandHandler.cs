...
{ 
    ...
    public class DeleteCurrencyRateCommandHandler : IRequestHandler<DeleteCurrencyRateCommand, DeleteRespObj>
    {
        private readonly ICommonRepository _repo;
        public DeleteCurrencyRateCommandHandler(ICommonRepository commonRepository)
        {
            _repo = commonRepository;
        }
        public async Task<DeleteRespObj> Handle(DeleteCurrencyRateCommand request, CancellationToken cancellationToken)
        {
            try
            {
                if (request.ItemsId.Count() > 0)
                {
                    ...
                    {
                        await _repo.DeleteCurrencyRateAsync(itemId);
                    }
                }
