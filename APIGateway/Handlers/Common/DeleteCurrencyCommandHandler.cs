...
{ 
    public class DeleteCurrencyCommandHandler : IRequestHandler<DeleteCurrencyCommand, DeleteRespObj>
    {
        private readonly ICommonRepository _repo;
        public DeleteCurrencyCommandHandler(ICommonRepository commonRepository)
        {
            _repo = commonRepository;
        }
        public async Task<DeleteRespObj> Handle(DeleteCurrencyCommand request, CancellationToken cancellationToken)
        {
            try
            {
                if (request.ItemsId.Count() > 0)
                {
                    ...
                    {
                        await _repo.DeleteCurrencyAsync(itemId);
                    }
                }
