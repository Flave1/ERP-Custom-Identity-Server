...
namespace APIGateway.Handlers.Common
{
    ...
    public class DeleteJobTitleCommandHandler : IRequestHandler<DeleteJobTitleCommand, DeleteRespObj>
    {
        private readonly ICommonRepository _repo;
        public DeleteJobTitleCommandHandler(ICommonRepository commonRepository)
        {
            _repo = commonRepository;
        }
        public async Task<DeleteRespObj> Handle(DeleteJobTitleCommand request, CancellationToken cancellationToken)
        {
            try
            {
                if (request.ItemsId.Count() > 0)
                {
                    ...
                    {
                        await _repo.DeleteJobTitleAsync(itemId);
                    }
                }
