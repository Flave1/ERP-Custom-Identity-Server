...
{ 
    public class DeleteStateCommandHandler : IRequestHandler<DeleteStateCommand, DeleteRespObj>
    {
        private readonly ICommonRepository _repo;
        public DeleteStateCommandHandler(ICommonRepository commonRepository)
        {
            _repo = commonRepository;
        }
        public async Task<DeleteRespObj> Handle(DeleteStateCommand request, CancellationToken cancellationToken)
        {
            try
            {
                if (request.ItemsId.Count() > 0)
                {
                    ...
                    {
                        await _repo.DeleteStateAsync(itemId);
                    }
                }
