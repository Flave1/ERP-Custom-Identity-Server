...
{ 
    public class DeleteBranchCommandHandler : IRequestHandler<DeleteBranchCommand, DeleteRespObj>
    {
        private readonly ICommonRepository _repo;
        public DeleteBranchCommandHandler(ICommonRepository commonRepository)
        {
            _repo = commonRepository;
        }
        public async Task<DeleteRespObj> Handle(DeleteBranchCommand request, CancellationToken cancellationToken)
        {
            try
            {
                if (request.ItemsId.Count() > 0)
                {
                    ...
                    {
                        await _repo.DeleteBranchAsync(itemId);
                    }
                }
